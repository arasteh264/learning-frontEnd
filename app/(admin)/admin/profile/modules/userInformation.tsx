...
import {
  EditOutlined, CameraOutlined, IdcardOutlined,
  PhoneOutlined, MailOutlined, CalendarOutlined, CrownOutlined,
} from "@ant-design/icons";
import { getUserProfile, UserProfile } from "@/src/services/user";
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3.5">
      ...
      <div className="flex flex-col gap-0.5 min-w-0">
        ...
        <span className="font-medium text-[#1C2B27] text-sm truncate">{value}</span>
      </div>
    </div>
  );
}
    ...
    <section className="w-full h-full flex flex-col items-center px-4 py-8 gap-6">
      <div className="w-full max-w-2xl flex flex-col items-center text-center gap-3">
        <div className="relative">
          <button type="button" onClick={() => fileInputRef.current?.click()}
            className="w-24 h-24 rounded-full overflow-hidden bg-[#EAF1EB] flex items-center justify-center text-3xl font-medium text-[#1EB35B] border-2 border-white shadow-sm"
            aria-label="آپلود عکس پروفایل">
            {avatarPreview ? (
              <Image src={avatarPreview} alt="عکس پروفایل" width={200} height={200}
      ...
      <div className="w-full max-w-2xl bg-white border border-gray-100 rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.04)] overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 divide-gray-100">
          <div className="sm:border-l border-gray-100">
            <ProfileField icon={<IdcardOutlined />} label="کد ملی"
              value={userData?.nationalId || "ثبت نشده"} />
          </div>
          <ProfileField icon={<PhoneOutlined />} label="شماره موبایل"
            value={userData?.phone || "ثبت نشده"} />
          <div className="sm:border-l border-gray-100">
            <ProfileField icon={<MailOutlined />} label="ایمیل"
              value={userData?.email || "ثبت نشده"} />
          </div>
          <ProfileField icon={<CalendarOutlined />} label="تاریخ تولد" value="ثبت نشده" />
        </div>
      </div>
      ...
      <div className="w-full max-w-2xl bg-[#FBF5E9] border border-[#EFE0BC] rounded-2xl px-5 py-4 flex items-center justify-between gap-4 flex-wrap">
        <div className="flex flex-col gap-0.5 text-right">
          <span className="text-sm font-medium text-[#7A5A1E]">به تیم اساتید ما بپیوندید</span>
          <span className="text-xs text-[#9A8048]">اگر تخصص خاصی دارید، می‌تونید درخواست تدریس بدید</span>
        </div>
      ...
      <TeacherRequestModal open={isTeacherModalOpen} onClose={() => setIsTeacherModalOpen(false)} />
    </section>
  );
}