...
function isValidNationalId(code: string) {
  ...
  const check = digits.at(9) ?? 0;
  const sum =
    digits.slice(0, 9).reduce((acc, num, i) => acc + num * (10 - i), 0) % 11;
...
export const profileSchema = z.object({
  name: z
    ...
    .regex(/^[\u0600-\u06FF\s]+$/, "نام فقط باید فارسی باشد"),
  lastName: z
    .string()
    .min(2, "نام خانوادگی خیلی کوتاه است")
    .regex(/^[\u0600-\u06FF\s]+$/, "نام خانوادگی فقط باید فارسی باشد"),
  nationalId: z
    .string()
